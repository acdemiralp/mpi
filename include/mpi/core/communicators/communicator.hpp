...
public:
  ...
  [[nodiscard]]
  status                                    send_receive        (const void*         sent            , const std::int32_t sent_size    , const data_type& sent_data_type    , const std::int32_t destination, 
                                                                       void*         received        , const std::int32_t received_size, const data_type& received_data_type, const std::int32_t source     , 
  ...
  [[nodiscard]]
  status                                    send_receive        (const sent_type&     sent           , const std::int32_t destination  , 
                                                                       received_type& received       , const std::int32_t source       , 
                                                                 const std::int32_t   send_tag    = 0, const std::int32_t receive_tag  = 0) const
  {
    using send_adapter    = container_adapter<sent_type    >;
    using receive_adapter = container_adapter<received_type>;
    return send_receive(
      static_cast<const void*>(send_adapter   ::data(sent    )), static_cast<std::int32_t>(send_adapter   ::size(sent    )), send_adapter   ::data_type(), destination,
      static_cast<      void*>(receive_adapter::data(received)), static_cast<std::int32_t>(receive_adapter::size(received)), receive_adapter::data_type(), source     ,
  ...
  [[nodiscard]]
  std::optional<std::pair<message, status>> async_probe_message (const std::int32_t source, const std::int32_t tag = 0) const
  {
    std::pair    result { message(), status() };
    std::int32_t exists;
  ...
  [[nodiscard]]
  request                                   async_broadcast     (void* data, const std::int32_t count, const data_type& data_type, const std::int32_t root = 0) const
  {
    request request;
  ...
  template <typename type>   
  [[nodiscard]]               
  request                                   async_broadcast     (type& data, const std::int32_t root = 0) const
  {
    using adapter = container_adapter<type>;
    return async_broadcast(static_cast<void*>(adapter::data(data)), static_cast<std::int32_t>(adapter::size(data)), adapter::data_type(), root);
  }
  ...
  [[nodiscard]]
  request                                   async_scatter       (const void*        sent    , const std::int32_t sent_size    , const data_type& sent_data_type    ,
                                                                       void*        received, const std::int32_t received_size, const data_type& received_data_type, 
                                                                 const std::int32_t root    = 0) const
  {
    request result;
  ...
  template <typename sent_type, typename received_type>
  [[nodiscard]]
  request                                   async_scatter       (sent_type& sent_data, received_type& received_data, const std::int32_t root = 0) const
  {
    using send_adapter    = container_adapter<sent_type>;
    using receive_adapter = container_adapter<received_type>;
    return async_scatter(
      static_cast<void*>(send_adapter   ::data(sent_data    )), static_cast<std::int32_t>(send_adapter   ::size(sent_data    )), send_adapter   ::data_type(), 
      static_cast<void*>(receive_adapter::data(received_data)), static_cast<std::int32_t>(receive_adapter::size(received_data)), receive_adapter::data_type(), root);
  }
