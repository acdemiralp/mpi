...
public:
 ...
 ~status           ()                    = default;
  ...
  [[nodiscard]]
  std::int32_t      get_count           (const data_type& type) const
  {
    std::int32_t result;
  ...
  void              set_element_count  (const data_type& type, const count element_count)
  {
    MPI_CHECK_ERROR_CODE(MPI_Status_set_elements_x, (&native_, type.native(), element_count))
  }
