...
public:
  ...
  {
    if      (filter == filter::exclude)
      MPI_Group_range_excl(that.native_, static_cast<std::int32_t>(ranges.size()), reinterpret_cast<std::int32_t(*)[3]>(const_cast<std::vector<range>&>(ranges).data()), &native_);
    else if (filter == filter::include)
      MPI_Group_range_incl(that.native_, static_cast<std::int32_t>(ranges.size()), reinterpret_cast<std::int32_t(*)[3]>(const_cast<std::vector<range>&>(ranges).data()), &native_);
    else
      throw std::runtime_error("Unknown group filter.");
  }
